...
public class LayoutBug {
    ...
    public static LayoutBugBuilder builder() {
        return new CustomLayoutBugBuilder();
    }

    private static class CustomLayoutBugBuilder extends LayoutBugBuilder {
        @Override
        public LayoutBug build() {
            LayoutBug layoutBug = super.build();

            // Ensure required fields have values
            if (layoutBug.getElementType() == null) {
                layoutBug.setElementType("unknown");
            }
            if (layoutBug.getDescription() == null) {
                layoutBug.setDescription("No description provided");
            }
            if (layoutBug.getSeverityLevel() == null) {
                layoutBug.setSeverityLevel(SeverityLevel.MEDIUM);
            }
            if (layoutBug.getHtmlSnippet() == null) {
                layoutBug.setHtmlSnippet("No HTML snippet available");
            }
            if (layoutBug.getIsActive() == null) {
                layoutBug.setIsActive(true);
            }
    ...
    private void beforeSave() {
        ...
        if (severityLevel == null) severityLevel = LayoutBug.SeverityLevel.MEDIUM;
